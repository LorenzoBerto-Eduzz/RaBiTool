...
- `v1.0.1` specifically hardens the worksheet-confirmation stage that could stay loading on `Confirmando aba Relatório de Tickets...`. It should show bounded attempts, then fail closed with a short actionable blocker if Excel/Chrome does not expose the active worksheet. Keep these blockers clear enough for colleagues to act on without needing the owner beside them, while keeping copied logs concise with the debug code at the end.
- `v1.0.2` keeps the worksheet-confirmation guard but fixes the RA processing wait so HugMe can keep polling for the Download button for the intended 420s instead of stopping after 10s. It also actively selects `Relatório de Tickets` when that worksheet tab is visible but not active, then confirms again before any Find/paste step.
...
- Existing releases exist through `v1.0.1`. Current requested release target is `v1.0.2`; future versions and future remote releases should be created only on explicit owner request.
