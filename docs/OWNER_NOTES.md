...
- During the Excel phase, other Chrome extensions can interfere if they open their own extension pages or try to control the same tab. RaBiTool now reconfirms and refocuses the tracked Planilha tab before attaching Chrome Debugger, attempts to return it to the workbook URL if it drifted, and retries before blocking. If another extension or DevTools is already controlling the Planilha tab, Chrome will not let RaBiTool share that debugger connection; the user should stop that recording/inspection and run RA > BI again.
