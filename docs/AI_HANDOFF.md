...
- Popup notice/log lines remain visually clean and show only the human-readable log text. Each notice still has a deterministic debug code derived from level/stage/text, for example `RBT-ERR-EXCEL-FIND-ANCHOR-...`; clicking a notice copies the human message with `Código: ...` appended at the end so the owner/colleagues can report precise failures back into Codex.
- New diagnostic behavior: final workflow notices are now normalized in `background/ra_bi_workflow.js` with stage, timestamp, compact result diagnostics, and a deterministic backend debug code. Clicking a notice copies the readable message plus `Etapa`, `Horário`, optional `Detalhes`, and `Código`, while the popup still visually shows only the readable message.
