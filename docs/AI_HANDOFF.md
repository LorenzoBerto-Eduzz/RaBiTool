...
- Auto-run cleanup differs from manual runs: when an auto-run RA > BI execution returns, RaBiTool automatically disables itself through the shared hard-shutdown path, closing the tracked HugMe/Planilha tabs and clearing workspace tracking. Manual RA > BI runs do not auto-close after completion.
