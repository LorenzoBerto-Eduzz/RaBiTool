...
- GitHub Releases exist through `v1.0.1`, and the current requested release target is `v1.0.2` with `RaBiTool.zip`.
...
- Options shortcut key elements use the same selected-day green (`#16a34a`) instead of the darker title green.
...
- `v1.0.1` adds a hard timeout around Chrome frame inspection and retries the `Relatório de Tickets` worksheet confirmation with visible attempt counters, so the workflow should fail closed with a clear `excel-worksheet` block instead of staying indefinitely on `Confirmando aba Relatório de Tickets...`. Owner alignment after release: colleague-facing status/blocker text must stay short, precise, and actionable so a colleague can understand what to try on their own, and clicking the log should still copy the concise message plus debug code for reporting.
- `v1.0.2` fixes the overly broad `v1.0.1` timeout regression by giving the HugMe report-processing watcher its full 420s window plus buffer instead of the shared 10s page-call timeout. It also makes the Excel worksheet guard more active: if `Relatório de Tickets` is visible but not selected, RaBiTool selects it, waits, and re-confirms before searching/pasting.
